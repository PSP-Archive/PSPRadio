...
jsaTextureCache::~jsaTextureCache()
{
	...
	{
		TextureIterator = m_TextureList.begin();
		/* free texture */
		vfree((void *)((*TextureIterator).offset));
...
bool jsaTextureCache::jsaTCacheStoreTexture(int ID, jsaTextureInfo *texture_info, void *tbuffer)
{
	...
	tsize		= (unsigned long)vgetMemorySize(texture_info->width, texture_info->height, texture_info->format);
	texture_address = (unsigned long)valloc(tsize);
	...
	{
		printf("Couldn't get VRAM for texture : %d\n", ID);
	}
	return ret_value;
}
...
bool jsaTextureCache::jsaTCacheSetTexture(int ID)
{
	...
	{
		printf("TC:No textures stored in VRAM.\n");
	}
	return found;
}
...
int jsaTextureCache::jsaTCacheLoadRawImage(const char* filename, u32 *ImageBuffer)
{
	...
	if (fhandle != NULL)
	{
		...
		if (filesize != -1)
		{
			...
			if (bytes != filesize)
			{
				printf("TC:Wrong filesize for %s : %d, %d\n", filename, filesize, bytes);
				return -1;
			}
		}
		else
		{
			printf("TC:Couldn't get filesize for %s\n", filename);
			return -1;
		}
		fclose(fhandle);
	}
	...
	{
		printf("TC:Couldn't load file : %s\n", filename);
		return -1;
	}
	return 0;
}

/* Load a set of textures to memory */
void jsaTextureCache::jsaTCacheLoadTextureSet(const char* path, jsaTextureFile *TextureSet, int count)
{
	char				filename[MAXPATHLEN];
	unsigned char 		*filebuffer;
	jsaTextureInfo		texture;
	...
	for (int i = 0 ; i < count ; i++)
	{
		...
		sprintf(filename, "%s/%s", path, TextureSet[i].filename);
		filebuffer = (unsigned char *) memalign(16, (int)(TextureSet[i].width * TextureSet[i].height * jsaTCacheTexturePixelSize(TextureSet[i].format)));

		if (TextureSet[i].filetype == FT_PNG)
		{
			if (jsaTCacheLoadPngImage((const char *)filename, (u32 *)filebuffer) == -1)
				{
				printf("Failed loading png file: %s\n", filename);
				free(filebuffer);
				continue;
				}
		}
		...
		{
			if (jsaTCacheLoadRawImage((const char *)filename, (u32 *)filebuffer) == -1)
				{
				printf("Failed loading raw file: %s\n", filename);
				free(filebuffer);
				continue;
				}
		}

		texture.format		= TextureSet[i].format;
		texture.width		= TextureSet[i].width;
		texture.height		= TextureSet[i].height;
		texture.swizzle		= TextureSet[i].swizzle;
		success = jsaTCacheStoreTexture(TextureSet[i].ID, &texture, filebuffer);
		if (!success)
		{
			printf("Failed storing texture in VRAM : %s\n", filename);
		}
		free(filebuffer);
	}
}
